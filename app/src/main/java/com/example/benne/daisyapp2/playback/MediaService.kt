...
import com.example.benne.daisyapp2.io.FileService
...
class MediaService :
    ...
    override fun onCreate() {
        super.onCreate()
        mediaProvider = MediaProvider(FileService())
        ...
        mediaSessionConnector = MediaSessionConnector(mediaSession).also {
            ...
            val playbackPreparer = DaisyPlaybackPreparer(
    ...
    override fun onLoadChildren(
        ...
        // parentId is a book
        if (mediaProvider.books.any { it.toMediaId() == parentMediaId }) {
            Log.d("aduioservice", "sending results for $parentMediaId ${mediaProvider.books.count()}")
            val book = mediaProvider.books
                    .first { it.toMediaId() == parentMediaId }
            result.sendResult(
                    book.navElements
                            .map { toMediaItem(it) }
            )
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        ...
        private fun updateNotification(state: PlaybackStateCompat) {
            ...
            if (updatedState == PlaybackStateCompat.STATE_STOPPED
    ...
    companion object {
        val TAG: String = MediaService::class.java.simpleName
        const val MEDIA_ROOT: String = "MEDIA_ROOT"
        const val ELEMENT_TYPE_KEY = "element_type_key"
        const val ELEMENT_TYPE_SUB_KEY = "element_type_sub_key"
    }
}
